...
final class NetworkLoggerPlugin: PluginType {
	...
	func willSend(_ request: RequestType, target: TargetType) {
		guard let httpRequest = request.request else {
			print("[Request] invalid httpRequest")
			return
		}
		
		self.requestTime = CFAbsoluteTimeGetCurrent()
	...
	func didReceive(_ result: Result<Response, MoyaError>, target: TargetType) {
		...
		if let requestTime {
			let duration = CFAbsoluteTimeGetCurrent() - requestTime
			let responseTimeLog = "[⏰ Response Time] \(duration)"
			
			print(responseTimeLog)
